...
void sample1() {
  ...
  float inc = 360.0 / push_constants.kG;
  float ca = inc / 9.0;
  float angle = (x + y) * ca;
  for (int k = 0; k < push_constants.kG; k ++) {
    bool flagF = false;
    bool flagB = false;
    ...
    for (int i = 0; i < push_constants.searchRange; i ++) {
      float t = step * float(i);
      float tx = x + ex * t,
        ty = y + ey * t;

      if (tx > 0.0 && tx < push_constants.scale.x
        && ty > 0.0 && ty < push_constants.scale.y
        && (!flagF || !flagB) ) {
          vec2 coord = vec2(tx, ty) / push_constants.scale;
  ...
  for (int i = 0; i < push_constants.kG; i ++) {
    if (i >= fIdx) {
      arrIdx(0, i, vec2(-1.0, -1.0));
      // arrIdx(0, i, gl_FragCoord.xy / push_constants.scale);
    }
  }
  for (int i = 0; i < push_constants.kG; i ++) {
    if (i >= bIdx) {
      arrIdx(1, i, vec2(-1.0, -1.0));
      // arrIdx(1, i, gl_FragCoord.xy / push_constants.scale);
    }
  }
  // FSamples[0] = gl_FragCoord.xy / push_constants.scale;
  // arrIdx(0, 0, gl_FragCoord.xy / push_constants.scale);
}


float e_p(vec2 coord) {
  float lx = coord.x * push_constants.scale.x - gl_FragCoord.x,
    ly = coord.y * push_constants.scale.y - gl_FragCoord.y,
    l = length(vec2(lx, ly));
...
float n_p(vec2 FCoord, vec2 BCoord) {
  ...
  for (int i = -1; i < 2; i ++) {
    for (int j = -1; j < 2; j ++) {
      float m = m_p((gl_FragCoord.xy + vec2(float(i), float(j)))/push_constants.scale, FCoord, BCoord);
      res += m * m;
    }
  }
...
FBPair gathering() {
  ...
  int idxI = 0, idxJ = 0;
  for (int i = 0; i < push_constants.kG; i ++) {
    for (int j = 0; j < push_constants.kG; j ++) {
      vec2 fs = arrIdx(0, i),
        bs = arrIdx(1, j);
...
void writeFB() {
  if (getTri().x > 0.0 && getTri().x < 1.0) {
    ...
    FBPair p = gathering();
    // F B component
    f_color = vec4(p.FCoord, p.BCoord);
  }
  else {
